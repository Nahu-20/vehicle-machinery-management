    ...
    for i in range(len(zone_ids)):
        for j in range(i + 1, len(zone_ids)):
            ...
            if s1.intersects(s2):
                ...
                # Check dimension / geometry type of intersection
                if inter.is_empty:
                    continue
                itype = inter.geom_type
                if itype in ['LineString', 'MultiLineString', 'Polygon', 'MultiPolygon', 'GeometryCollection']:
                    # Check if there is a linear component
                    length = inter.length
                    if length > 1e-7: # shared boundary segment length
                        edge_adjacent[z1].append(z2)
                        edge_adjacent[z2].append(z1)
                    else:
                        # point intersection within geometry collection
