/**
 * CLI: seed published Investment CMS prototype datasets for /investment map.
 *
 * Usage:
 *   set SEED_ADMIN_EMAIL=admin@oromiaagri.gov.et
 *   set SEED_ADMIN_PASSWORD=...
 *   npm run seed:investment-prototype
 */
...
import { seedPrototypeInvestmentMapData } from '../src/services/investment/investmentPrototypeSeedService';
...
async function main() {
  ...
  console.log(`Seeding as ${staff.email || email} (${staff.role})…`);
  const result = await seedPrototypeInvestmentMapData(staff);
  console.log(JSON.stringify(result, null, 2));
  console.log('Done. Open /investment map and select coffee/wheat/maize layers.');
  process.exit(0);
}
