...
export const AdminSignInPage: React.FC = () => {
  ...
  const { user, firebaseUser, staffUser, status, signIn, signInDemoUser, loading, authError, clearError, isFirebaseConfigured } = useAuth();
  ...
  const isAuthenticated = !!(user || firebaseUser || staffUser || status === 'demoAuthorized');
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white dark:bg-slate-900/90 backdrop-blur-xl py-8 px-5 sm:px-10 shadow-xl rounded-3xl border border-slate-200 dark:border-slate-800">
          ...
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
            ...
            <button
              type="button"
              onClick={() => signInDemoUser('superAdmin')}
              className="w-full py-3.5 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm shadow-md flex items-center justify-center gap-2 transition-all cursor-pointer mt-2"
            >
              <span>Bypass Login (Super Admin)</span>
            </button>
          </form>
