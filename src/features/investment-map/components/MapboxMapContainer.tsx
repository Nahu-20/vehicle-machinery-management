  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <div className="relative overflow-hidden rounded-2xl" style={{ height }}>
        {loading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center gap-2 bg-slate-900/60 text-sm text-emerald-200">
        ...
        <button
          onClick={toggle3D}
          className="absolute left-3 top-3 z-10 inline-flex items-center gap-1.5 rounded-lg border border-emerald-700/40 bg-slate-900/85 px-2.5 py-1.5 text-[11px] font-bold text-emerald-200 shadow-lg backdrop-blur transition-colors hover:bg-slate-900 cursor-pointer"
        ...
        {hover && (
          <div
            className="pointer-events-none absolute z-20 -translate-x-1/2 -translate-y-[calc(100%+14px)] rounded-lg border border-emerald-500/40 bg-slate-900/95 px-3 py-2 text-xs shadow-xl backdrop-blur"
          ...
          >
            ...
            {hover.value !== null ? (
              <div className="mt-0.5 text-emerald-300">
                ...
                {hover.classLabel && (
                  <span className="ml-1.5 text-emerald-500/80">· {hover.classLabel}</span>
                )}
              </div>
            ) : (
      ...
      <div className="flex flex-wrap items-center gap-2 text-[11px]">
        <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 font-mono dark:border-emerald-800/60 dark:bg-emerald-950/40">
          <MapPin className="h-3 w-3 text-emerald-600 dark:text-emerald-400" />
          <span className="font-bold text-emerald-700 dark:text-emerald-400">Mapbox GL Engine</span>
          <span className="text-slate-500 dark:text-slate-400">
            | {featureCount} Candidate Polygons
          </span>
        </span>
        {checksumOk && (
          <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 font-semibold text-emerald-700 dark:border-emerald-800/60 dark:bg-emerald-950/40 dark:text-emerald-400">
