...
import { PublicAchievement } from '../../types/achievement';
import { subscribeToHomepageAchievements } from '../../services/achievementService';

export const AchievementsSection: React.FC = () => {
  const { language, t, getLocalizedText } = useLanguage();

  const [featured, setFeatured] = useState<PublicAchievement | null>(null);
  const [secondaryAchievements, setSecondaryAchievements] = useState<PublicAchievement[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [isEmpty, setIsEmpty] = useState<boolean>(false);

  useEffect(() => {
    const unsub = subscribeToHomepageAchievements((data) => {
      setFeatured(data.featuredAchievement);
      setSecondaryAchievements(data.latestAchievements);
  ...
  return (
    <section id="achievements-impact" className="py-16 md:py-24 bg-slate-50 dark:bg-slate-900/50 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        ) : isEmpty || (!featured && secondaryAchievements.length === 0) ? (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 mb-12 text-center border border-slate-200/80 dark:border-slate-700/80 shadow-sm">
            <Award className="w-12 h-12 text-slate-400 mx-auto mb-3" />
            <p className="text-slate-600 dark:text-slate-300 font-medium text-sm">
              No published achievements found.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
            
            {/* Featured Achievement Card (7 cols on lg) */}
            {featured && (
              <div className="lg:col-span-7">
                ...
                >
                  ...
                  <div className="absolute inset-0 z-0">
                    <img
                      src={featured.featuredImage || 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&auto=format&fit=crop&q=80'}
                      alt={getLocalizedText(featured.imageAlt) || getLocalizedText(featured.title)}
                  ...
                  <div className="absolute top-4 left-4 z-10 flex flex-wrap gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-950/80 text-emerald-300 border border-emerald-500/30 font-medium text-xs backdrop-blur-md capitalize">
                  ...
                  <div className="relative z-10 p-6 sm:p-8 flex flex-col justify-end mt-auto">
                    ...
                    {featured.metrics && featured.metrics.length > 0 && (
                      <div className="grid grid-cols-3 gap-3 p-4 rounded-xl bg-white/10 backdrop-blur-md border border-white/15 mb-6">
                        {featured.metrics.slice(0, 3).map((metric, idx) => (
                          <div key={metric.id || idx} className="text-center">
                    ...
                    <div className="pt-2 border-t border-white/15 flex items-center justify-between">
                      <span className="text-xs text-slate-400 flex items-center gap-1">
                        ...
                        <span className="truncate max-w-[200px] sm:max-w-xs">{getLocalizedText(featured.responsibleOffice)}</span>
                      </span>
                      <Link
            ...
            <div className={`lg:col-span-${featured ? '5' : '12'} flex flex-col gap-4`}>
              ...
              {secondaryAchievements.map((item, idx) => {
                ...
                const itemSummary = getLocalizedText(item.summary);
                const itemAlt = getLocalizedText(item.imageAlt) || itemTitle;
                const itemImage = item.featuredImage || 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&auto=format&fit=crop&q=80';
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-2 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider mb-1 capitalize">
                      ...
                      <div className="mt-3 pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between">
                        <span className="text-[11px] text-slate-500 dark:text-slate-400 truncate max-w-[150px]">
                          {item.metrics?.[0] ? `${item.metrics[0].value} ${getLocalizedText(item.metrics[0].label)}` : ''}
                        </span>

                        <Link
                          to={`/achievements/${item.slug}`}
                          className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 dark:text-emerald-400 group-hover:translate-x-0.5 transition-transform shrink-0"
                        >
                          <span>{t('view_full_story')}</span>
